...
public class UpLoadPhotos   {
    ...
    public static String init(String[] path){
        String serverURL = "http://172.20.10.3:8000/upload/";
...
//        String path = Environment.getExternalStorageDirectory().getAbsolutePath() + "/KK.jpeg";
        ...
        for(int i =0; i < path.length; i++){
            FileBody bin = null;
            if(i == 0)
                bin = new FileBody(new File(path[i]));
            else{
                bin = new FileBody(new File(path[i].substring(1)));

            }
            Log.e("jianming","jiaming"+path[i]);
